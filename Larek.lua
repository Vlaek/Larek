...
function main()
	...
	if ini[LarekConfig] == nil then
		sampAddChatMessage(u8:decode" [Larek] Откалибруйте время в /larek", main_color)
		ini = inicfg.load({
			[LarekConfig] = {
				time = tonumber(0),
				X = tonumber(0),
		...
		if caption == u8:decode"Изменение названия" then
			if result then
				if button == 1 then
					...
					inicfg.save(ini, directIni)
					sampAddChatMessage(u8:decode" [Larek] Новое название: {FFFFFF}" .. input, main_color)
					ShowDialog(1)
				else
					ShowDialog(1)
				end
...
if ini3[LarekTime].time1 == u8:decode"Неизвестно" then
		color[1] = "{808080}"
	else
		if timerM[1] == 1 then
			color[1] = "{d10000}"
	...
	if ini3[LarekTime].time2 == u8:decode"Неизвестно" then
		color[2] = "{808080}"
	else
		if timerM[2] == 1 then
			color[2] = "{d10000}"
	...
	if ini3[LarekTime].time3 == u8:decode"Неизвестно" then
		color[3] = "{808080}"
	else
		if timerM[3] == 1 then
			color[3] = "{d10000}"
	...
	if ini3[LarekTime].time4 == u8:decode"Неизвестно" then
		color[4] = "{808080}"
	else
		if timerM[4] == 1 then
			color[4] = "{d10000}"
	...
	if ini3[LarekTime].time5 == u8:decode"Неизвестно" then
		color[5] = "{808080}"
	else
		if timerM[5] == 1 then
			color[5] = "{d10000}"
	...
	if ini3[LarekTime].time6 == u8:decode"Неизвестно" then
		color[6] = "{808080}"
	else
		if timerM[6] == 1 then
			color[6] = "{d10000}"
	...
	if ini3[LarekTime].time7 == u8:decode"Неизвестно" then
		color[7] = "{808080}"
	else
		if timerM[7] == 1 then
			color[7] = "{d10000}"
	...
	if ini3[LarekTime].time8 == u8:decode"Неизвестно" then
		color[8] = "{808080}"
	else
		if timerM[8] == 1 then
			color[8] = "{d10000}"
	...
	if ini3[LarekTime].time9 == u8:decode"Неизвестно" then
		color[9] = "{808080}"
	else
		if timerM[9] == 1 then
			color[9] = "{d10000}"
	...
	if ini3[LarekTime].time10 == u8:decode"Неизвестно" then
		color[10] = "{808080}"
	else
		if timerM[10] == 1 then
			color[10] = "{d10000}"
	...
	if ini3[LarekTime].time11 == u8:decode"Неизвестно" then
		color[11] = "{808080}"
	else
		if timerM[11] == 1 then
			color[11] = "{d10000}"
	...
	if ini3[LarekTime].time12 == u8:decode"Неизвестно" then
		color[12] = "{808080}"
	else
		if timerM[12] == 1 then
			color[12] = "{d10000}"
	...
	if ini3[LarekTime].time13 == u8:decode"Неизвестно" then
		color[13] = "{808080}"
	else
		if timerM[13] == 1 then
			color[13] = "{d10000}"
	...
	if ini3[LarekTime].time14 == u8:decode"Неизвестно" then
		color[14] = "{808080}"
	else
		if timerM[14] == 1 then
			color[14] = "{d10000}"
	...
	if ini3[LarekTime].time15 == u8:decode"Неизвестно" then
		color[15] = "{808080}"
	else
		if timerM[15] == 1 then
			color[15] = "{d10000}"
	...
	if ini3[LarekTime].time16 == u8:decode"Неизвестно" then
		color[16] = "{808080}"
	else
		if timerM[16] == 1 then
			color[16] = "{d10000}"
...
function ShowDialog(int, dtext, dinput, string_or_number, ini1, ini2)
	...
	if int == 1 then
		...
		sampShowDialog(1488, u8:decode'Larek: Список', text, u8:decode"Выбрать", u8:decode"Выход", 4)
	end
	if int == 3 then
	...
	if int == 19 then
		LarekMoney = string.format('LarekMoney-%s', my_name)
		ini2 = inicfg.load(LarekMoney, directIni2)
		sampShowDialog(1506, u8:decode'Статистика', u8:decode"Денег награблено: \t" .. ini2[LarekMoney].money .. u8:decode"\nМагазинов ограблено: \t" .. ini2[LarekMoney].count
		.. u8:decode"\nОграблений в 2: \t" .. ini2[LarekMoney].count2 .. u8:decode"\nОграблений в 3: \t" .. ini2[LarekMoney].count3 .. u8:decode"\nОграблений в 4: \t" .. ini2[LarekMoney].count4 .. u8:decode"\nОграблений в 1: \t" .. ini2[LarekMoney].count1, u8:decode"Выбрать", u8:decode"Выход", 4)
	end
end

function MagazTimeFunction()
	if MagazTime[1] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name1 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[1] = false
	end
	if MagazTime[2] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name2 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[2] = false
	end
	if MagazTime[3] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name3 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[3] = false
	end
	if MagazTime[4] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name4 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[4] = false
	end
	if MagazTime[5] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name5 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[5] = false
	end
	if MagazTime[6] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name6 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[6] = false
	end
	if MagazTime[7] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name7 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[7] = false
	end
	if MagazTime[8] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name8 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[8] = false
	end
	if MagazTime[9] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name9 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[9] = false
	end
	if MagazTime[10] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name10 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[10] = false
	end
	if MagazTime[11] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name11 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[11] = false
	end
	if MagazTime[12] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name12 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[12] = false
	end
	if MagazTime[13] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name13 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[13] = false
	end
	if MagazTime[14] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name14 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[14] = false
	end
	if MagazTime[15] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name15 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[15] = false
	end
	if MagazTime[16] == true then
		sampAddChatMessage(u8:decode" [Larek] Ларёк {FFFFFF}" .. ini[LarekName].Name16 .. u8:decode" {FF0000}снова доступен для ограбления!", main_color)
		MagazTime[16] = false
	end
end
